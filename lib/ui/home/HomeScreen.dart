...
import 'package:uber_eats_driver_app/ui/chat/ChatScreen.dart';
import 'package:uber_eats_driver_app/ui/home/pick_order.dart';
...
class HomeScreenState extends State<HomeScreen> {
  ...
  void setSourceIcon() async {
    driverIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(46, 46)), 'assets/images/location_black3x.png');
    storeIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(46, 46)), 'assets/images/location_orange3x.png');
    customerIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(46, 46)), 'assets/images/location_orange3x.png');
  }
  ...
  updateDriverOrder() async {
    ...
    await FirebaseFirestore.instance
        .collection(ORDERS)
        .where('status', whereIn: ["Order Accepted", "Driver Rejected"])
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (user == null) {
              return Center(
                child: showEmptyState('Unexpected Error'.tr(), 'Failed to retrieve user data.'.tr()),
              );
            } else {
              ...
              if (user.isActive) {
                ...
                return Stack(
                  ...
                  children: [
                    ...
                    if (user.inProgressOrderID != null && currentOrder != null) buildOrderActionsCard(user)
                  ],
                );
              } else {
                return Center(
                  child: showEmptyState(
                      'You are offline'.tr(), 'Go online in order to start getting delivery requests from customers and vendors.'.tr(),
                      ...
                      action: () => showDialog(
                            ...
                            builder: (ctx) => AlertDialog(
                              ...
                              content: Text.rich(TextSpan(children: [
                                TextSpan(text: "Foodie Driver App ".tr(), style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            return showEmptyState('Unexpected Error'.tr(), 'Connection to server failed.'.tr());
          }
        },
      ),
    );
  ...
  acceptOrder(User user) async {
    currentOrder = await fireStoreUtils.getOrderByID(user.orderRequestData!.id);
    if (currentOrder!.status == ORDER_STATUS_DRIVER_PENDING) {
      ...
      await FireStoreUtils.sendFcmMessage("Delivery Agent Assigned.", user.firstName + " " + user.lastName + " will deliver Your Order.",
      ...
      final snack = SnackBar(
        content: Text(
          'This order is assign to another driver',
  ...
  addPolyLinesAndMarkersToMap(User user) async {
    ...
    if (currentOrder!.status == ORDER_STATUS_SHIPPED || currentOrder!.status == ORDER_STATUS_DRIVER_ACCEPTED) {
      mapMarkers.add(
        Marker(markerId: MarkerId('driverMarker'), position: polylineCoordinates.first, icon: driverIcon),
      );
      mapMarkers.add(
        Marker(
    ...
    polylines.add(
      Polyline(
        polylineId: PolylineId('polyline_id_${currentOrder!.driver!.userID}_to_${currentOrder!.vendor.title}'),
  ...
  getCurrentOrder() async {
    currentOrder = await fireStoreUtils.getOrderByID(MyAppState.currentUser!.inProgressOrderID ?? '');
            ...
            if (currentOrder!.status == ORDER_STATUS_SHIPPED || currentOrder!.status == ORDER_STATUS_DRIVER_ACCEPTED)
              ListTile(
                title: Text(
                  title,
                  style: TextStyle(
                      color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff000000), fontFamily: "Poppinsm", letterSpacing: 0.5),
                ),
                subtitle: Padding(
                  ...
                  child: Text(
                    ...
                    maxLines: 2,
                    style: TextStyle(
                        color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff000000), fontFamily: "Poppinsr", letterSpacing: 0.5),
                  ),
                ),
              ...
              ListTile(
                ...
                title: Row(
                  children: [
                    Text(
                      'ORDER ID'.tr(),
                      style: TextStyle(
                          color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff555555), fontFamily: "Poppinsr", letterSpacing: 0.5),
                    ),
                    SizedBox(
                      width: 110,
                      child: Text(
                ...
                subtitle: Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                        color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff333333), fontFamily: "Poppinsm", letterSpacing: 0.5),
                  ),
                ),
                // trailing: TextButton.icon(
                //     style: TextButton.styleFrom(
                //       shape: RoundedRectangleBorder(
                //         borderRadius: BorderRadius.circular(6.0),
                //         side: BorderSide(color: Color(0xffFF683A)),
                //       ),
                //       padding: EdgeInsets.zero,
                //       minimumSize: Size(85, 30),
                //       alignment: Alignment.center,
              ...
              ListTile(
                ...
                subtitle: Padding(
                  ...
                  child: Text(
                    ...
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                        color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(0xff333333), fontFamily: "Poppinsr", letterSpacing: 0.5),
                  ),
                ),
                trailing: Column(
                  mainAxisAlignment: MainAxisAlignment.start,
                                ...
                                Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Text(
                                      ...
                                      style: TextStyle(
                                          color: isDarkMode(context) ? Colors.white : Color(0xff3DAE7D),
                                          fontFamily: 'Poppinsm',
                                          letterSpacing: 0.5),
                                    ),
                                  ),
  ...
  completeOrder() async {
    ...
    await FireStoreUtils.sendFcmMessage(
        "Order Complete".tr(), "Our Delivery agent delivered order.".tr(), currentOrder!.author.fcmToken, null);
    MyAppState.currentUser!.inProgressOrderID = null;
    currentOrder = null;
    await FireStoreUtils.updateCurrentUser(MyAppState.currentUser!);
    hideProgress();
